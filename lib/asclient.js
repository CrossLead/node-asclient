...
clientProto.provision = function() {
  ...
  return new Promise(function(resolve, reject) {
    asRequest(self.opts, 'POST', 'Provision', 'provision').
      then(function(res) {
	if (!res.body || !res.body.Provision){
	  return reject(new Error('No provision response received'));
	}

        if (res.body.Provision.Status[0] !== '1') {
          return resolve(res.body.Provision.Status[0]);
        }
	var policies = res.body.Provision.Policies[0].Policy[0].Data[0].EASProvisionDoc[0];
	Object.keys(policies).forEach(policy => self.policies[policy] = parseInt(policies[policy]));

	var templateToUse = 'provisionConfirm';
	if(res.body.Provision.RemoteWipe != null) {
	  templateToUse = 'provisionConfirmRemoteWipe';
	  self.opts.requestedRemoteWipe = true;
	}

	self.opts.policyKey = res.body.Provision.Policies[0].Policy[0].PolicyKey[0];
	asRequest(self.opts, 'POST', 'Provision', templateToUse).
	  then(function(res) {
	    if (!res.body || !res.body.Provision) {
	      return reject(new Error('Provisioning failed'));
            ...
            if (res.body.Provision.Status[0] !== '1') {
              const status = parseInt(res.body.Provision.Status[0]);
...
clientProto.folderSync = function() {
	...
	return new Promise(function(resolve, reject) {
		...
		then(function(res) {
                        if (!res.body) {
		          return reject(new Error('Folder Synchronization failed: no body'));
                        } else if (!res.body.FolderSync) {
                          return reject(new Error('Folder Synchronization failed: body missing FolderSync'));
                        } else if (res.body.FolderSync.Status[0] !== '1') {
                          return reject(new Error('Folder Synchronization failed with status: ' + res.body.FolderSync.Status[0]));
		        }
